...
| 模式 | 适用场景 | 案例 |
|------|----------|------|
| 接力（串行） | A 的输出是 B 的输入 | 抓取文章 → 总结内容 |
| 分工（并行） | 同一输入，多个 Skill 处理不同维度 | 内容准确性、可读性、发布规范同时审查 |
| 编排（编排 Skill） | 固定流程、频繁重复、有依赖关系 | 视频下载→过滤→变声→打包上传 |
...
**Skill 的稳定性依赖于下游 API 的稳定性。** 如果被调用的接口改了返回格式，Skill 会静默地产出错误数据。要在失败处理中建立机制感知下游变更。
...
每个 Skill 至少准备 20 条测试用例，按四类分布：

- 5 条应该触发（验证核心功能）
- 5 条不该触发（验证排除条件）
- 5 条边界（部分满足条件，验证路由精度）
- 5 条异常（缺字段/冲突/超时，验证失败处理）

迭代规则：每次修改 Skill，必须跑完这 20 条，确保没有退化。这和 Prompt 工程里"对抗式调优"的回归测试纪律完全一致。

#### 11.2 Linter + Rules 自动化
...
- [ ] 路由契约写了吗？触发条件和排除条件都显式声明了吗？
- [ ] description 是否一句话说清了"做什么 + 何时用 + 何时不用 + 输出什么"？
- [ ] 触发词是否覆盖了用户可能的各种说法（中/英/口语/正式）？
- [ ] 必需输入的每个字段都写了"缺失时的默认行为"吗？
- [ ] SKILL.md 是否控制在 500 行以内？超出部分拆子文档了吗？
- [ ] 确定性操作是否剥离给了 scripts，而非交给模型"发挥"？
- [ ] 自由度设置和任务容错率匹配吗？
...
- [ ] 一个 Skill 是否只做一件事？有没有可以拆出去单独复用的步骤？
- [ ] 输出格式是通用的 Markdown/JSON 还是私有结构？
- [ ] 两个 Skill 之间有没有职责重叠？边界声明是否显式化？

**测试与迭代：**

- [ ] 是否有 20+ 条测试用例（触发/排除/边界/异常各 5 条）？
- [ ] 每次修改后是否跑了全量回归？
...
```
阶段一：设计
  ├── 识别场景关键字段，确定"绝不能删"的核心项
  ├── 选定 Prompt 骨架结构（Role/Constraints/Input/Rules/Output）
  ├── 设计路由契约（触发条件 + 排除条件 + 必需输入）
  ├── 判断 Skill 形态（定时任务 / 会话触发）
  ├── 按任务容错率设定自由度（高风险锁脚本 / 低风险交模型）
  └── 控制总长度，让模型做判断而非精算

阶段二：构建
  ├── 写结构化 Prompt 模板（输出格式直接可套用）
  ├── 准备 Few-shot 示例（特别是边界反例）
  ├── 按 SKILL.md 结构模板填充各章节
  ├── 确定性操作剥离给 scripts，三层渐进式加载
  ├── 实现四环执行闭环 + 幂等性保证
  └── 设计单一职责边界与协作接口

阶段三：验证与迭代
  ├── 跑测试集（Prompt: 全量回归 / Skill: 20 条四类用例）
  ├── 从输出反推漏洞，归类误判模式，用反例堵边界
